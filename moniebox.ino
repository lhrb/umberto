...
bool is_playing(byte* receive_buffer) {
    query_playback_status();
    if(receive()) {
      if (receive_buffer[8] == 1) {
...
void loop() {
  int touchSensorVal = digitalRead(TOUCH_SENSOR_PIN);
  bool isPlaying = is_playing(receive_buffer);
  Serial.println(touchSensorVal);
  if(touchSensorVal && isPlaying) {
    delay(500);
    return;
  } else if (!touchSensorVal && isPlaying) {
    stop_playback();
    return;
  } else if(!touchSensorVal) {
    return;
  }
